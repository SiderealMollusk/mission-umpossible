import fs from 'fs';
import path from 'path';
import dotenv from 'dotenv';
import jwt from 'jsonwebtoken';
import crypto from 'crypto';
import { fileURLToPath } from 'url';

// Resolve correct paths
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const envPath = path.resolve(__dirname, '../../.env');

// Load and parse .env
const env = dotenv.parse(fs.readFileSync(envPath));

// Ensure SUPABASE_JWT_SECRET exists
if (!env.SUPABASE_JWT_SECRET) {
  console.log('No SUPABASE_JWT_SECRET found, generating a new one...');
  env.SUPABASE_JWT_SECRET = crypto.randomBytes(32).toString('hex');
}

// Create new service_role JWT
const serviceRolePayload = {
  role: 'service_role'
};

const newServiceToken = jwt.sign(serviceRolePayload, env.SUPABASE_JWT_SECRET, {
  algorithm: 'HS256'
});

// Replace in memory
env.SUPABASE_SERVICE_ROLE_KEY = newServiceToken;

// Write updated .env
const updatedEnv = Object.entries(env)
  .map(([key, value]) => `${key}=${value}`)
  .join('\n') + '\n';

fs.writeFileSync(envPath, updatedEnv);

console.log('✅ Updated SUPABASE_SERVICE_ROLE_KEY in .env');
